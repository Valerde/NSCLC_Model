...
class PredictServiceImpl:
    ...
    def executor(self, image_path, label_path):
        ...
        result = self.extractFeatures(image_path, label_path, extractor=extractor,
                                      file_name='/root/IdeaProjects/model/output/features/')
        return result

    def predict(self, path):
        data = pd.read_excel(path, index_col=0)
        features_name = np.load('/root/IdeaProjects/model/output/features_name.npy', allow_pickle=True)

        s = data[data.columns.intersection(features_name)]

        model = load_model('/root/IdeaProjects/model/output/model.pkl')

        predict = model.predict(s)
        return predict

    def predictNSCLC(self, image_path, label_path):
        feature_path = self.executor(image_path, label_path)
        result = self.predict(feature_path)
        print(feature_path)
        print(result)
...
a = PredictServiceImpl()
a.predictNSCLC('/root/IdeaProjects/model/input/images/cc_nj_67_0.nrrd',
               '/root/IdeaProjects/model/input/images/cc_nj_67_0-label.nrrd')
